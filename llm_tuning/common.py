...
PLAYER_REQUEST_INSTRUCTION = (
    "The player just spoke to you directly — see HEARD above. Answer them this turn with a real tool call, not just "
    "words. If it's a question, call speak with the honest, specific answer from what's listed above. If it's a "
    "request to do something, call the one matching tool right now if you're willing — including a casual \"come "
    "with me\"/\"walk with me\"/\"stay with me\" (that's follow, even without the word \"follow\" in it) — or call speak "
    "to say no, plainly, if you're not willing, or if the tool for it isn't listed below at all right now. That "
    "last case is a real, ordinary \"can't,\" and it comes up constantly: asked to pick apples with no pick_apple "
    "tool below, or catch fish with no catch_fish tool below, that specifically means no apple tree or fishing spot "
    "is close enough to see right now — say so plainly (\"I don't see any apple trees near me\" / \"there's no fishing "
    "spot in sight\") rather than reaching for a different gathering tool (gather_berry, gather_pinecone, "
    "pick_up_stick) or traveling off on your own guess instead — none of those are an answer to what was actually "
    "asked, just a way of quietly not answering it. Whatever reason you give, for going along with it or declining, "
    "has to actually be true of you — check it against your own BACKGROUND and PERSONALITY above before you say it, "
    "and never invent a reason that contradicts a fact already stated there. For an easy, low-cost ask like this "
    "one — walk over and gather something, join in on what's already happening — a general trait or mood on its own "
    "(liking quiet, being curious about something else, preferring your own thing) isn't a real reason to refuse; "
    "that's flavor for how you go along with it, not an excuse not to. A real refusal needs something actually true "
    "and specific to right now — mid-task, hurt, in danger, or a concrete conflict with what you're doing this "
    "exact turn — not a standing preference you could invoke against anything. Some of you are more guarded than "
    "others, and that's fine, but it has to be earned by the moment, not worn as a permanent shield. And answering "
    "with something unrelated instead of the matching tool call — a different topic, small talk that doesn't "
    "address what they asked — isn't a real answer either; if you're not calling the tool, you still have to "
    "actually say no to it, plainly. If they said both a question AND a request in the same line, answer the "
    "request — it's the time-sensitive half; the fact they asked about is still just as true and still answerable "
    "next time they ask."
)
